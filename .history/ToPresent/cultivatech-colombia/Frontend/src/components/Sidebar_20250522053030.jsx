...
export default function Sidebar({ currentPage, onNav }) {
  ...
  return (
    <nav className="cultiva-earth-main p-4 space-y-2 md:w-64 md:min-h-screen hidden md:block">
      {navItems.map(item => (
        <button
          key={item.key}
          className={`nav-button w-full text-left btn btn-nav ${currentPage === item.key ? "active" : ""}`}
